...
class DataFetcher:
    def __init__(self, api_key, period):
    ...
    def fetch_data(self, stock_code, exchange_code):
        try:
            data_request = requests.get(f"{self.base_url}{stock_code}.{exchange_code}?api_token={self.api_key}&period={self.period}&fmt=json").json()
        except:
